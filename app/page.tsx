...
import { MeetingType, Summary, Section } from '@/lib/types';
...
const MEETING_TYPES = [
  ...
  { id: 'process' as MeetingType, icon: '🔄', label: 'סיכום תהליך', sub: 'מעבר לליווי מתמשך' },
];
...
function SectionCard({ section }: { section: Section }) {
  return (
    <div style={{ marginBottom: '22px' }}>
      {section.title && (
        <div style={{ fontWeight: 'bold', color: GREEN, fontSize: '15px', borderBottom: `2px solid ${LIGHT_GREEN}`, paddingBottom: '5px', marginBottom: '10px' }}>
          {section.title}
        </div>
      )}
      <div style={{ fontSize: '13.5px', color: '#333', lineHeight: '1.8', direction: 'rtl', textAlign: 'right' }}>
...
export default function Home() {
  ...
  const handleDownload = () => {
    ...
    const html = buildWordHtml(result, name, d);
    ...
    <div dir="rtl" style={{ background: 'transparent', fontFamily: "'Segoe UI', Arial, sans-serif" }}>
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '24px 18px' }}>
        ...
        <div style={{ background: '#fff', borderRadius: '14px', padding: '26px', boxShadow: '0 1px 4px rgba(0,0,0,0.07)', marginBottom: '18px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              {MEETING_TYPES.map(t => (
                <button key={t.id} onClick={() => { setMeetingType(t.id); setResult(null); setError(''); }}
                  style={{ flex: '1 1 calc(25% - 10px)', minWidth: '140px', padding: '13px 10px', border: `2px solid ${meetingType === t.id ? GREEN : '#e0e0e0'}`, borderRadius: '10px', background: meetingType === t.id ? LIGHT_GREEN : '#fff', cursor: 'pointer', textAlign: 'right', direction: 'rtl', transition: 'all 0.15s' }}>
          ...
          {loading && <div style={{ textAlign: 'center', color: '#aaa', fontSize: '12px', marginTop: '8px' }}>קלוד קורא את הפגישה ובונה את הסיכום, כ-30-60 שניות</div>}
        </div>
        ...
        {result && (
          <div style={{ background: '#fff', borderRadius: '14px', padding: '26px', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
              <div>
                <div style={{ fontWeight: 'bold', fontSize: '16px', color: '#222' }}>{result.meetingTitle || (clientName || result.clientName || 'לקוח')}</div>
                <div style={{ fontSize: '12px', color: '#aaa' }}>{date || result.date || ''}</div>
              </div>
